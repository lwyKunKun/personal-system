...
export default function PersonalRecentPage() {
  ...
  useEffect(() => {
    setItems(getPersonalState().recent)
  }, [])

  const clearRecent = () => {
    const next = getPersonalState()
    const updated = { ...next, recent: [] }
  ...
  return (
    <div className="rounded-2xl border border-white/8 bg-[#0d1728]/80 p-6">
      <div className="mb-4 flex items-center justify-between gap-3">
        ...
        <button type="button" onClick={clearRecent} className="rounded-xl border border-white/8 bg-[#101b2d] px-3 py-2 text-sm text-slate-100">
          清空
        </button>
      </div>
          ...
          items.map((item) => (
            <div key={item} className="flex items-center justify-between gap-3 rounded-xl border border-white/8 bg-[#0b1220] p-3">
              ...
              <button
                type="button"
                onClick={() => {
                  const next = removePersonalEntry("recent", item)
