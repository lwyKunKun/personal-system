...
export default function PersonalFavoritesPage() {
  ...
  useEffect(() => {
    setItems(getPersonalState().favorites)
  }, [])
  ...
  return (
    <div className="rounded-2xl border border-white/8 bg-[#0d1728]/80 p-6">
      <div className="mb-4 flex items-center justify-between gap-3">
        ...
        <button type="button" onClick={() => handleAdd(window.location.pathname)} className="rounded-xl bg-[#f68f4d] px-3 py-2 text-sm font-medium text-white">
          收藏当前页
        </button>
      </div>
      ...
      <div className="mb-4 flex gap-3">
        <input
          ...
          placeholder="输入路径，例如 /system/settings"
          className="flex-1 rounded-xl border border-white/8 bg-[#0b1220] px-3 py-2.5 text-slate-100 outline-none"
        />
        <button type="button" onClick={() => handleAdd()} className="rounded-xl border border-white/8 bg-[#101b2d] px-3 py-2 text-sm text-slate-100">
          添加收藏
        </button>
      </div>
          ...
          items.map((item) => (
            <div key={item} className="flex items-center justify-between gap-3 rounded-xl border border-white/8 bg-[#0b1220] p-3">
              ...
              <button
                type="button"
                onClick={() => {
                  const next = removePersonalEntry("favorites", item)
                  setItems(next.favorites)
                }}
