...
export default function PersonalCommonPage() {
  ...
  useEffect(() => {
    ...
    } else { setItems(state.common) }
  }, [])
  const addCommon = (path: string) => { const next = addPersonalEntry("common", path, 12); setItems(next.common) }
  return (
    <WorkspaceShell>
      <div className="rounded-2xl border border-white/8 bg-[#0d1728]/80 p-6">
        <div className="mb-4 flex items-center justify-between gap-3">
          ...
          <button type="button" onClick={() => addCommon("/projects/ai")} className="rounded-xl bg-[#f68f4d] px-3 py-2 text-sm font-medium text-white">添加 AI 入口</button>
        </div>
          ...
          {items.length === 0 ? (
            ...
            items.map((item) => (
              <div key={item} className="flex items-center justify-between gap-3 rounded-xl border border-white/8 bg-[#0b1220] p-3">
                ...
                <button type="button" onClick={() => { const next = removePersonalEntry("common", item); setItems(next.common) }} className="text-xs text-rose-200">删除</button>
              </div>
            ))
          )}
        </div>
