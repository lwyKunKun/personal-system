...
const MENU_TEMPLATES = [{ id: "default", label: "默认导航", items: DEFAULT_SIDEBAR_ITEMS }]
...
// 快速新建路由弹窗
function QuickCreateRouteModal({
  defaultGroup = "project",
  onClose,
  onCreated,
}: {
  defaultGroup?: "system" | "project" | "personal"
  onClose: () => void
  onCreated: (route: RouteDefinition) => void
}) {
  const [form, setForm] = useState({ title: "", path: "", type: "page" as RouteType, iconName: "" })
  const handleCreate = () => {
      ...
      <div className="w-full max-w-md rounded-2xl border border-white/10 bg-[#0d1628] p-6" onClick={(e) => e.stopPropagation()}>
        ...
        <div className="space-y-3">
          ...
          <input
            ...
            placeholder="路径（如 /projects/demo 或 https://...）"
            className="w-full rounded-xl border border-white/8 bg-[#0b1220] px-3 py-2.5 text-sm text-slate-100 outline-none"
          />
          <select
            value={form.type}
            onChange={(e) => setForm({ ...form, type: e.target.value as RouteType })}
            className="w-full rounded-xl border border-white/8 bg-[#0b1220] px-3 py-2.5 text-sm text-slate-100 outline-none"
          >
        ...
        <div className="mt-5 flex justify-end gap-2">
          <button type="button" onClick={onClose} className="rounded-xl border border-white/8 bg-[#101b2d] px-4 py-2 text-sm text-slate-200">
            取消
          </button>
          <button type="button" onClick={handleCreate} className="rounded-xl bg-[#f68f4d] px-4 py-2 text-sm font-medium text-white">
            创建并关联
          </button>
        </div>
      </div>
...
export default function MenusManagerPage() {
  ...
  const [showQuickCreate, setShowQuickCreate] = useState<{ defaultGroup?: "system" | "project" | "personal"; onCreated: (r: RouteDefinition) => void } | null>(null)
  ...
  useEffect(() => {
    ...
    return () => {
      window.removeEventListener("vibe:sidebar-updated", load)
      window.removeEventListener("vibe:routes-updated", load)
    }
  }, [])
  ...
  const totalVisibleNodes = countVisibleNodes(previewTree),
    totalMenuNodes = countVisibleNodes(items)
  ...
  const applyPermissionTemplate = (template: { roles: string[]; visible?: boolean }) => {
    ...
    setItems((prev) => {
      ...
      const apply = (node: SidebarMenuItem): SidebarMenuItem => ({
        ...node,
        roles: template.roles.length ? [...template.roles] : undefined,
        visible: template.visible ?? node.visible ?? true,
        children: node.children?.map(apply),
      })
      ref.parent[ref.index] = apply(ref.parent[ref.index])
      saveStoredSidebarItems(next)
      return next
    })
  ...
  const applyMenuTemplate = (template: SidebarMenuItem[]) => {
    ...
    setSelectedPath(template.length > 0 ? [0] : null)
    setStatusText(`已应用模板`)
  }
  ...
  const addItem = () => {
    ...
    const next = [...items, { label, icon: newIcon.trim() || undefined, iconName: newIconName.trim() || undefined, visible: newVisible, sortOrder: items.length }]
    persist(next)
    setNewLabel("")
    setNewIcon("")
    setNewIconName("")
    setNewVisible(true)
    setStatusText("已添加菜单分组")
  }
  ...
  const addChildItem = (path: number[]) => {
    ...
    setItems((prev) => {
      ...
      target.children = [
        ...(target.children ?? []),
        { label, icon: newIcon.trim() || undefined, iconName: newIconName.trim() || undefined, visible: newVisible, sortOrder: target.children?.length ?? 0 },
      ]
      saveStoredSidebarItems(next)
      return next
    })
  ...
  const updateItem = (path: number[], patch: Partial<SidebarMenuItem>) => {
    setItems((prev) => {
      ...
      if (!ref || ref.index < 0) return prev
      ref.parent[ref.index] = { ...ref.parent[ref.index], ...patch }
      saveStoredSidebarItems(next)
      return next
    })
  }
  ...
  const moveItem = (path: number[], direction: "up" | "down") => {
    setItems((prev) => {
      ...
      const ref = getParentAndIndex(next, path)
      if (!ref || ref.index < 0) return prev
      const tIdx = direction === "up" ? ref.index - 1 : ref.index + 1
      if (tIdx < 0 || tIdx >= ref.parent.length) return prev
      ;[ref.parent[ref.index], ref.parent[tIdx]] = [ref.parent[tIdx], ref.parent[ref.index]]
      saveStoredSidebarItems(next)
      return next
    })
  }
  ...
  const importJson = () => {
    try {
      ...
      if (!Array.isArray(parsed) || parsed.some((i) => !i.label)) throw new Error("格式错误")
      persist(parsed)
      setStatusText("导入成功")
    } catch {
      setStatusText("导入失败：JSON 格式错误")
    }
  }
  const handleReset = () => {
  ...
  const flattenTreeKeys = (nodes: SidebarMenuItem[], path: number[] = []): string[] =>
    nodes.flatMap((item, idx) => {
      const cur = [...path, idx]
      const key = cur.join(".")
      return [key, ...(item.children?.length ? flattenTreeKeys(item.children, cur) : [])]
    })
  ...
  const reorderTree = (source: number[], target: number[]) => {
    setItems((prev) => {
      const next = JSON.parse(JSON.stringify(prev)) as SidebarMenuItem[]
      const sp = getParentAndIndex(next, source)
      const tp = getParentAndIndex(next, target)
      ...
      tp.parent.splice(tp.index, 0, moved)
      saveStoredSidebarItems(next)
      return next
    })
  }
      ...
      return (
        <div key={key} className="space-y-2">
          <div
            ...
            onDrop={(e) => {
              e.preventDefault() /* drag source not tracked */
            }}
          ...
          >
            <button
              type="button"
              onClick={() => hasChildren && toggleNode(cur)}
            ...
            <button type="button" onClick={() => setSelectedPath(cur)} className="flex flex-1 items-center gap-2 overflow-hidden text-left">
              <span className="flex h-8 w-8 items-center justify-center rounded-lg bg-[#111d2d] text-[#ffb476]">
                {renderLucidePreview(item.iconName || route?.iconName) || item.icon || "◌"}
              </span>
            ...
            <button type="button" onClick={() => addChildItem(cur)} className="rounded-md border border-white/8 bg-[#101b2d] px-2 py-1 text-[10px] text-slate-200">
              +
            </button>
          </div>
  ...
  const PropertySection = ({ title, keyName, children }: { title: string; keyName: string; children: React.ReactNode }) => (
    <div className="rounded-2xl border border-white/8 bg-[#091321] p-3">
      <button type="button" onClick={() => toggleSection(keyName)} className="flex w-full items-center justify-between text-left text-sm font-medium text-white">
        <span>{title}</span>
        <span className="text-[10px] uppercase tracking-[0.14em] text-slate-400">{(propertySections as any)[keyName] !== false ? "收起" : "展开"}</span>
      </button>
  ...
  return (
    <SystemShell>
      <div className="space-y-5">
            ...
            <div className="mb-3 flex flex-wrap gap-2">
              {MENU_TEMPLATES.map((t) => (
                <button
                  key={t.id}
                  type="button"
                  onClick={() => applyMenuTemplate(t.items)}
                ...
                {LUCIDE_ICON_OPTIONS.map((n) => {
                  ...
                  return (
                    <button
                      ...
                      type="button"
                      title={n}
                      onClick={() => setNewIconName(n)}
                      className={`flex h-8 w-8 items-center justify-center rounded-lg border ${act ? "border-[#f68f4d] bg-[#f68f4d]/15 text-[#ffb476]" : "border-white/8 bg-[#0b1220] text-slate-200 hover:border-[#f68f4d]/40"}`}
                    >
        ...
        <div className="grid gap-6 xl:grid-cols-[0.9fr_1.1fr]">
          <div className="rounded-2xl border border-white/8 bg-[#0d1628] p-4">
            ...
            <div className="mb-3 flex flex-wrap gap-2">
              ...
              <Link href="/system/routes" className="rounded-lg border border-[#5ea2ff]/40 bg-[#5ea2ff]/10 px-2.5 py-1.5 text-[11px] text-blue-200">
                管理路由 →
              </Link>
            </div>
            ...
            {selectedItem ? (
              <div className="space-y-4">
                <div className="flex items-center justify-between gap-3 rounded-2xl border border-white/8 bg-[#091321] p-3">
                  <div className="flex items-center gap-3">
                    <span className="flex h-10 w-10 items-center justify-center rounded-xl bg-[#121d32] text-[#ffb476]">
                      {renderLucidePreview(selectedItem.iconName || selectedItemRoute?.iconName, "h-5 w-5") || selectedItem.icon || "◌"}
                    </span>
                    <div>
                  ...
                  <div className="grid gap-3 md:grid-cols-2">
                    ...
                    <label className="space-y-1.5">
                      ...
                      <select
                        ...
                        onChange={(e) => selectedPath && updateItem(selectedPath, { group: e.target.value as any })}
                        className="w-full rounded-xl border border-white/8 bg-[#0b1220] px-3 py-2.5 text-sm text-slate-100 outline-none"
                      >
                    ...
                    <label className="space-y-1.5 block">
                      ...
                      <div className="flex gap-2">
                        <select
                          ...
                          onChange={(e) => selectedPath && updateItem(selectedPath, { routeId: e.target.value || undefined, path: undefined })}
                          className="flex-1 rounded-xl border border-white/8 bg-[#0b1220] px-3 py-2.5 text-sm text-slate-100 outline-none"
                        >
                        ...
                        <button
                          ...
                          onClick={() =>
                            setShowQuickCreate({
                              defaultGroup: (selectedItem.group as any) ?? "project",
                              onCreated: (r) => {
                                selectedPath && updateItem(selectedPath, { routeId: r.id })
                                setRoutes(getStoredRoutes())
                              },
                            })
                          }
                          className="rounded-xl border border-[#f68f4d]/40 bg-[#f68f4d]/10 px-3 py-2 text-xs text-[#ffb476]"
                        >
                          + 新建
                        </button>
                      </div>
                    ...
                    {selectedItemRoute && (
                      <div className="rounded-xl border border-white/8 bg-[#0b1220] p-3 text-xs text-slate-300">
                        <div className="flex items-center justify-between">
                          <span>
                            路径：<code className="text-slate-200">{selectedItemRoute.path}</code>
                          </span>
                          <Link
                            href={selectedItemRoute.type === "link" ? selectedItemRoute.url || selectedItemRoute.path : selectedItemRoute.path}
                            target={selectedItemRoute.target === "_blank" ? "_blank" : undefined}
                            className="text-[#f68f4d] hover:underline"
                          >
                            预览 →
                          </Link>
                        </div>
                        <div className="mt-1 text-slate-500">
                          类型：{selectedItemRoute.type}
                          {selectedItemRoute.builtin ? " · 内置" : ""}
                        </div>
                      </div>
                    )}
                ...
                <PropertySection title="访问控制" keyName="access">
                  <div className="space-y-3">
                    <div className="flex flex-wrap gap-2">
                      {ROLE_STRATEGIES.map((s) => (
                        <button
                          key={s.id}
                          type="button"
                          onClick={() => applyRoleStrategy(s.roles)}
                          className="rounded-lg border border-white/8 bg-[#101b2d] px-2.5 py-1.5 text-[11px] text-slate-200"
                        >
                          {s.label}
                        </button>
                    ...
                    <div className="space-y-2">
                      ...
                      {ROLE_TEMPLATES.map((t) => (
                        <button
                          key={t.id}
                          type="button"
                          onClick={() => applyPermissionTemplate(t)}
                          className="block w-full rounded-xl border border-white/8 bg-[#0b1220] p-2.5 text-left"
                        >
                          <div className="flex items-center justify-between">
                            <span className="text-sm font-medium text-slate-100">{t.label}</span>
                            <span className="text-[10px] uppercase tracking-wide text-slate-400">{t.visible === false ? "隐藏" : "可见"}</span>
                          </div>
        ...
        <div className="grid gap-6 lg:grid-cols-[1.2fr_0.8fr]">
          <div className="rounded-2xl border border-white/8 bg-[#0d1628] p-4">
            ...
            <div className="space-y-3">
              <button type="button" onClick={copyExport} className="w-full rounded-xl border border-white/8 bg-[#101b2d] px-3 py-2 text-sm text-slate-100">
                复制 JSON
              </button>
              <textarea
                value={jsonText}
      ...
      {showQuickCreate && (
        <QuickCreateRouteModal
          defaultGroup={showQuickCreate.defaultGroup}
          onClose={() => setShowQuickCreate(null)}
          onCreated={(r) => {
            const cb = showQuickCreate.onCreated
