...
// 快速新建路由弹窗
function QuickCreateRouteModal({ defaultGroup = "project", onClose, onCreated }: { defaultGroup?: "system" | "project" | "personal"; onClose: () => void; onCreated: (route: RouteDefinition) => void }) {
  ...
  const handleCreate = () => {
    ...
    const route = createRoute({
      ...
      iconName: form.iconName.trim() || undefined, group: defaultGroup,
      visible: true,
    })
    onCreated(route)
  }
...
export default function MenusManagerPage() {
  ...
  const addItem = () => {
    ...
    persist(next); setNewLabel(""); setNewIcon(""); setNewIconName(""); setNewVisible(true); setStatusText("已添加菜单分组")
  }
  ...
  return (
    <SystemShell>
        ...
        <div className="grid gap-4 lg:grid-cols-[1.2fr_0.8fr]">
          <div className="rounded-2xl border border-white/8 bg-[#0d1628] p-4">
            ...
            <div className="grid gap-3 md:grid-cols-[1fr_auto]">
              ...
              <button type="button" onClick={addItem} className="rounded-xl bg-[#f68f4d] px-4 py-2.5 text-sm font-medium text-white">添加顶级分组</button>
            </div>
                  ...
                  <div className="space-y-3">
                    <label className="space-y-1.5 block">
                      ...
                      <div className="flex gap-2">
                        ...
                        <button type="button" onClick={() => setShowQuickCreate({ defaultGroup: (selectedItem.group as any) ?? "project", onCreated: (r) => { selectedPath && updateItem(selectedPath, { routeId: r.id }); setRoutes(getStoredRoutes()) } })} className="rounded-xl border border-[#f68f4d]/40 bg-[#f68f4d]/10 px-3 py-2 text-xs text-[#ffb476]">+ 新建</button>
                      </div>
                    </label>
                    {selectedItemRoute && (
      ...
      {showQuickCreate && <QuickCreateRouteModal defaultGroup={showQuickCreate.defaultGroup} onClose={() => setShowQuickCreate(null)} onCreated={(r) => { const cb = showQuickCreate.onCreated; setShowQuickCreate(null); cb(r) }} />}
    </SystemShell>
  )
}
